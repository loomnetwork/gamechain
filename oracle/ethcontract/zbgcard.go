...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *ZBGCardTransferTokenIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(ZBGCardTransferToken)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
func (_ZBGCard *ZBGCardFilterer) FilterTransferToken(opts *bind.FilterOpts, from []common.Address, to []common.Address, tokenId []*big.Int) (*ZBGCardTransferTokenIterator, error) {
	...
	logs, sub, err := _ZBGCard.contract.FilterLogs(opts, "TransferToken", fromRule, toRule, tokenIdRule)
	if err != nil {
		return nil, err
	}
	return &ZBGCardTransferTokenIterator{contract: _ZBGCard.contract, event: "TransferToken", logs: logs, sub: sub}, nil
}
...
func (_ZBGCard *ZBGCardFilterer) WatchTransferToken(opts *bind.WatchOpts, sink chan<- *ZBGCardTransferToken, from []common.Address, to []common.Address, tokenId []*big.Int) (event.Subscription, error) {
	...
	logs, sub, err := _ZBGCard.contract.WatchLogs(opts, "TransferToken", fromRule, toRule, tokenIdRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(ZBGCardTransferToken)
				if err := _ZBGCard.contract.UnpackLog(event, "TransferToken", log); err != nil {
					return err
				}
				event.Raw = log
