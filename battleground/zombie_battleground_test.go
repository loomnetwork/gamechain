...
func setup(c *ZombieBattleground, pubKeyHex string, addr *loom.Address, ctx *contract.Context, t *testing.T) *plugin.FakeContext {
	debugEnabled = true
...
func TestMatchMakingTimeout(t *testing.T) {
	...
	// setup ctx
	fc := setup(c, pubKeyHexString, &addr, &ctx, t)
	now := time.Now()
	fc.SetTime(now)
